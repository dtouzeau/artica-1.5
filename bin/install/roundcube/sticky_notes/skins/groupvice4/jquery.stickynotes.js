(function(jQuery) {
	...
	jQuery.fn.stickyNotes.Random = function () {
	  A=0;
	  B=50;
    var pos = A + (B-A)*Math.random()  // num is random, from A to B
	...
	//find me: modified for Roundcube sticky_notes plugin (save on unload aswell)
	jQuery.fn.stickyNotes.saveEditedNotes = function() {
	...
	jQuery.fn.stickyNotes.prepareContainer = function(container) {
		...
		if (jQuery.fn.stickyNotes.options.controls) {
		  //find me: modified for Roundcube sticky_notes plugin (added localization)
			jQuery("#sticky-container").html('<button id="add_note">' + rcmail.get_label('sticky_notes.add_note') + '</button>');			
    ...
    $(window).unload( function(e) {
      jQuery.fn.stickyNotes.saveEditedNotes();
    });		
	...
	jQuery.fn.stickyNotes.createNote = function() {
		var pos_x = jQuery.fn.stickyNotes.Random();
		var pos_y = jQuery.fn.stickyNotes.Random();		
		var note_id = jQuery.fn.stickyNotes.notes.length + 1;
		//find me: mods by roundcube sticky_notes plugin
		var myTextarea = document.createElement("textarea");
		var created = new Date();
    var myText = document.createTextNode(created.toLocaleString() + ":\r\n");
    myTextarea.appendChild(myText);
    var _note_content = jQuery(myTextarea);
    //end mods
	...
	jQuery.fn.stickyNotes.stopEditing = function(note_id) {
		...
		//find me: mods for roundcube sticky_notes plugin	
		if(note.text){
	...
	jQuery.fn.stickyNotes.deleteNote = function(delete_button) {
	  //find me: modified for Roundcube sticky_notes plugin (added try/catch)
	  try{
    ...
    var _div_background = jQuery.fn.stickyNotes.createNoteBackground();
		...
		var _div_delete = 	$(document.createElement('div'))
							...
							.click(function(){jQuery.fn.stickyNotes.deleteNote(this);});
		var _div_wrap 	= 	$(document.createElement('div'))
							.css({'position':'absolute','top':note.pos_x,'left':note.pos_y, 'float': 'left',"width":note.width,"height":note.height})
	...
	jQuery.fn.stickyNotes.movedNote = function(note_id) {
		var note = jQuery.fn.stickyNotes.getNote(note_id);
		note.pos_y=jQuery("#note-" + note_id).css("left").replace(/px/, "");
		note.pos_x=jQuery("#note-" + note_id).css("top").replace(/px/, "");
  ...
  jQuery.fn.stickyNotes.createNoteBackground = function() {
    ...
    if (jQuery.browser.msie && jQuery.browser.version <= 6)  {
      //find me: modified for Roundcube sticky_notes plugin (modified path)
      background = $(document.createElement('div')).addClass("background").html('<img src="plugins/sticky_notes/skins/default/images/spacer.gif" class="stretch" style="margin-top:5px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'plugins/sticky_notes/skins/default/images/sticky-bg.png\',sizingMethod=\'scale\'");" alt="" />');
    } else {
      //find me: modified for Roundcube sticky_notes plugin (modified path)   
      background = $(document.createElement('div')).addClass("background").html('<img src="plugins/sticky_notes/skins/default/images/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
    }
    return background;
  }
