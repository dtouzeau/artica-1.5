...
uses
Classes, SysUtils,Process,strutils,IniFiles,unix,BaseUnix,confiles,Logs,RegExpr in 'RegExpr.pas',md5;
...
begin
    ...
    if FileExists('/usr/sbin/useradd') then begin
           ...
           cmd:=cmd + username;
           Shell('/usr/sbin/useradd ' + cmd + ' >/tmp/useradd.' +username + '.' + groupname + ' 2>&1');
    end;
end;
