...
private
       ...
       function  MAIL_IMAP_CLIENT_VERSION():string;
       procedure MAIL_IMAP_CLIENT_INSTALL();
...
procedure myconf.MAIL_IMAP_CLIENT_CHECK();
var
   version:string;
   versionint:integer;
begin
versionint:=0;
if not SYS.CHECK_PERL_MODULES('Mail::IMAPClient') then begin
            logs.Debuglogs('Starting......: Mail::IMAPClient does not exists');
            MAIL_IMAP_CLIENT_INSTALL();
     end else begin
       version:=trim(MAIL_IMAP_CLIENT_VERSION());
       logs.Debuglogs('Starting......: Mail::IMAPClient v'+version);
       version:=AnsiReplaceText(version,'.','');
       if TryStrToInt(version,versionint) then begin
          if versionint<314 then MAIL_IMAP_CLIENT_INSTALL();
       end;

   end;
end;
//##############################################################################
function myconf.MAIL_IMAP_CLIENT_VERSION():string;
var  version:string;
begin
version:=SYS.GET_CACHE_VERSION('Mail::IMAPClient');
if length(version)>0 then exit(version);
result:=SYS.CHECK_PERL_MODULES_VERSION('Mail::IMAPClient');
SYS.SET_CACHE_VERSION('Mail::IMAPClient',result);
end;
//##############################################################################
procedure myconf.MAIL_IMAP_CLIENT_INSTALL();
begin
   if not fileExists(get_ARTICA_PHP_PATH()+'/bin/install/Mail-IMAPClient-3.14.tar.gz') then begin
       logs.Syslogs('Starting......:  WARNING !!! cannot install Mail::IMAPClient unable to stat '+get_ARTICA_PHP_PATH()+'/bin/install/Mail-IMAPClient-3.14.tar.gz');
       exit;
   end;


       forceDirectories('/tmp/artica/install');
       fpsystem('tar -xf '+get_ARTICA_PHP_PATH()+'/bin/install/Mail-IMAPClient-3.14.tar.gz -C /tmp/artica/install/');

       if not FileExists('/tmp/artica/install/Mail-IMAPClient-3.14/Makefile.PL') then begin
        logs.Syslogs('Starting......:  WARNING !!! cannot extract '+get_ARTICA_PHP_PATH()+'/bin/install/Mail-IMAPClient-3.14.tar.gz');
        exit;
       end;

      SetCurrentDir('/tmp/artica/install/Mail-IMAPClient-3.14');
      fpsystem('/bin/touch /tmp/artica/install/Mail-IMAPClient-3.14/test.txt');
      fpsystem(SYS.LOCATE_PERL_BIN()+' Makefile.PL');
      fpsystem('make');
      fpsystem('make install');
      SetCurrentDir('/root');
      fpsystem('/bin/rm -rf /tmp/artica/install/Mail-IMAPClient-3.14');
