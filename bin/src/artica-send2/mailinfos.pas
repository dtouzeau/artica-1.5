...
uses
Classes, SysUtils,Process,strutils,IniFiles,unix,BaseUnix,logs,RegExpr in 'RegExpr.pas',sendldap,mimemess, mimepart,md5,geoip;
...
type
  ...
  TTMailInfo = record //shared data between component, listen thread and handler
    ...
    mailman_listname:string;
    mailman_listtype:string;
    mailman_robot:boolean;
    WHITE_LISTED:boolean;
    HeaderDatas:TstringList;
    ldap:ldapinfos;
...
public
      ...
      function PrependSubject(messages:TTMailInfo;tag:string):TTMailInfo;
...
function TMailInfo.PrependSubject(messages:TTMailInfo;tag:string):TTMailInfo;
var
   Mime:TMimeMess;
   i:integer;
   RegExpr:TRegExpr;
...
begin
  result:=messages;
  Mime:=TMimeMess.Create;
  Mime.Lines.LoadFromFile(messages.FilePath_source);
  Mime.DecodeMessage;
  Mime.Header.Subject:=tag + Mime.Header.Subject;
  Mime.EncodeMessage;
  Mime.Lines.SaveToFile(messages.FilePath_source);
  Mime.Free;
...
begin
   ...
   if ParamStr(1)='-geoip' then begin
    ...
    messages.mailman_robot:=false;
    messages.WHITE_LISTED:=false;
    messages.HeaderDatas:=TstringList.Create;
    AntiSpamRate:=0;
...
// File method -----------------------------------------------------------------
    if messages.InPutMethod=false then begin
            LOGS.logs('Input method=false');
            file_ext:=ExtractFileExt(FilePath);
            LOGS.logs('Extension=' + file_ext);
            ...
            if messages.AsEmailRelay=true then begin
               ...
               if not FileExists(HeadFilePath) then begin
                      LOGS.logs('Unable to stat ' + HeadFilePath);
                      tmp_file:=ChangeFileExt(FilePath,'.envelope.bad');
                      LOGS.logs('Get header from source location ' + tmp_file);

                      if FileExists(tmp_file) then begin messages.HeadFilePath:=tmp_file;
                      end else begin
                          LOGS.logs('unable to get headers from ' + FilePath);
               ...
               if Not FileExists(HeadFilePath) then begin
                  LOGS.logs('TMailInfo::Scan [' + FilePath + '] unable to stat ' + HeadFilePath);
            ...
            if Not FileExists(HeadFilePath) then begin
               LOGS.logs('unable to stat headers from ' + HeadFilePath);
               if  messages.AsEmailRelay=true then begin
                   EmailRelay_RebuildHeaderfromFile(FilePath);
               end else begin
                   exit;
...
// File method -----------------------------------------------------------------
    if messages.InPutMethod=True then begin
       ...
       LOGS.logs('Mime Parsing ' + IntToStr(Mime.Lines.Count) + ' lines');
    end else begin
...
//------------------------------------------------------------------------------
    for i:=0 to Mime.Header.CustomHeaders.Count-1 do begin;
        messages.HeaderDatas.Add(Mime.Header.CustomHeaders.Strings[i]);
    end;
...
begin
  zLOGS:=Tlogs.Create;
  zLOGS.logs('retreive headers from ' + messagepath);
...
RegExpr.Expression:='^From:.+<(.+?)>';
  for i:=0 to FILE_DATA.Count -1 do begin
    ...
    RegExpr.free;
    zLOGS.logs('retreive <' + mailfrom + '> to <' + mailto + '> from ' + messagepath);
    FILE_DATA.Clear;
