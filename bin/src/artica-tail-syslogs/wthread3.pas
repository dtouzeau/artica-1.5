...
implementation
//##############################################################################
constructor TSampleThread3.Create();
begin
...
logs:=tlogs.Create;
  logs.Debuglogs('wthread3 Execute :: -> Start');
...
begin
 ...
 if not FileExists('/usr/local/ap-mailfilter3/log/filter.log') then exit;
...
logs.Debuglogs('wthread3:: filter_log() :: -> ' +maillog_path + ' old_filesize=' + intToStr(old_filesize) + ',old_lines=' + intToStr(old_lines) + '('+intToStr(fileSize) + ' bytes)');

if fileSize<old_filesize then begin
    logs.Debuglogs('wthread3:: filter_log() :: -> ' +maillog_path + '-> purge');
     old_filesize:=0;
     old_lines:=0;
end;
...
if old_filesize=fileSize then exit;
   ...
   logs.Debuglogs('wthread3:: filter_log() :: -> ' + maillog_path + '-> analyze (fileSize='+ IntToStr(fileSize)+' bytes)');
...
begin
  ...
  if not RegExpr.Exec(line) then exit;
        logs.Debuglogs('wthread3:: ParseLine() :: -> ' +line );
     ...
     logs.Debuglogs('wthread3:: ParseLine() :: '+ from + ' ' + time + '('+spam_stat +')');
