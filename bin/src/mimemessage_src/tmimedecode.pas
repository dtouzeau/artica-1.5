...
uses
Classes, SysUtils,variants,IniFiles,RegExpr in 'RegExpr.pas',mimemess,mimepart,ldap,smtpsend,oldlinux,Process,logs,strutils,md5,db,sqlite3ds;
...
private
     ...
     function release_mail_from_path(mail_from:string;mail_to:string;message_path:string):boolean;
...
public
    ...
    function SendHTMLMail(FilePath:string):boolean;
    function ReleaseMailmd5(md5:string):boolean;
    function releaseallmailfrommd5(md5:string):boolean;
    lasterror:string;
...
//##############################################################################
function ArticamimeDecode.SendHTMLMail(FilePath:string):boolean;
var
  RegExpr:TRegExpr;
...
begin
   ...
   RegExpr.Expression:='<FROM>(.+?)</FROM>\s+<TO>(.+?)</TO>\s+<SUBJECT>(.+?)</SUBJECT>\s+<MAILBODY>(.+?)</MAILBODY>';
   T:=TStringlist.Create;
   if not FileExists(filepath) then exit;
   T.LoadFromFile(filepath);
   if not RegExpr.Exec(T.Text) then begin
      writeln('unable to regex ' + filepath);
      writeln(T.Text);
      exit;
   end;
   
   writeln('TO: ',RegExpr.Match[2]);
   writeln('FROM: ',RegExpr.Match[1]);
   writeln('Subject: ',RegExpr.Match[3]);
   L:=TstringList.Create;
   Z:=TstringList.Create;
   l.Add(RegExpr.Match[4]);
   mm:=TMIMEMess.Create;
   mm.header.from:=RegExpr.Match[1];
   mm.header.ToList.add(RegExpr.Match[2]);
   mm.header.subject:=RegExpr.Match[3];
   mm.Header.XMailer:='Microsoft Office Outlook 11';
   mm.Header.CustomHeaders.Add('X-MimeOLE: Produced By Microsoft MimeOLE V6.00.2900.3198');
   Part := mm.AddPartHTML(l,nil);
   mm.EncodeMessage;
 ...
 if not SMTP.MailFrom(RegExpr.Match[1],Length(mm.Lines.Text)) then begin
       lasterror:=SMTP.FullResult.Text;
       exit(false);
     end;

     if not SMTP.MailTo(RegExpr.Match[2]) then begin
        lasterror:=SMTP.FullResult.Text;
        exit(false);
     end;

     if not SMTP.MailData(mm.Lines) then begin
        lasterror:=SMTP.FullResult.Text;
        exit(false);
     end;
     lasterror:=SMTP.FullResult.Text;
     SMTP.Logout;
     shell('/bin/rm ' + FilePath);
     exit(true);

   

end;
function ArticamimeDecode.ReleaseMailmd5(md5:string):boolean;
var
   db:TSqlite3Dataset;
   SQL:string;
   L:TstringList;
...
begin
  D:=False;
  D:=COMMANDLINE_PARAMETERS('debug');
   SQL:='SELECT mail_from,zMD5,subject,message_path,mail_to FROM messages WHERE zMD5="' + md5 +'" LIMIT 0,1';
   if D then writeln(sql);
   db:=TSqlite3Dataset.Create(nil);
   db.FileName:='/usr/share/artica-postfix/LocalDatabases/artica_database.db';
   writeln('Check MD5 ' + md5);
   db.SQL:=SQL;
   db.Open;
...
while not db.EOF do begin
   ...
   mail_to:=db.FieldByName('mail_to').AsString;
   db.Next;
end;

    if Not FileExists(message_path) then begin
          writeln('This email is already deleted from disk');
          db.QuickQuery('UPDATE messages SET Deleted=1 where zMD5="' + md5 + '"');
          db.Close;
          exit;
    end;


    writeln('From:<strong>' + mail_from + '</strong>');
    writeln('Subject:<strong>'+db.FieldByName('subject').AsString + '</strong>');
    release_mail_from_path(mail_from,mail_to,message_path);
...
//##############################################################################
function ArticamimeDecode.releaseallmailfrommd5(md5:string):boolean;
var
   db:TSqlite3Dataset;
   SQL:string;
   L:TstringList;
...
while not db.EOF do begin
 ...
 db.Close;
 SQL:='SELECT ID,mail_from,subject,quarantine,Deleted,message_path,mail_to FROM messages WHERE mail_to="'+mail_to+'" AND mail_from="'+mail_from +'" AND quarantine=1 AND Deleted IS NULL';
...
while not db.EOF do begin
   ...
   if FileExists(message_path) then begin
      writeln('<div style="border:1px solid #CCCCCC;width:80%;padding:5px;margin:5px">');
      writeln('From:<strong>' + mail_from + '</strong>');
      writeln('Subject:<strong>'+db.FieldByName('subject').AsString + '</strong>');
      release_mail_from_path(mail_from,mail_to,message_path);
      writeln('</div>');
...
function ArticamimeDecode.release_mail_from_path(mail_from:string;mail_to:string;message_path:string):boolean;
var
   FILEDATA:TStringList;
   SMTP:TSMTPSend;
begin
    ...
    FILEDATA.LoadFromFile(message_path);
 ...
 if not SMTP.MailFrom(mail_from,Length(FILEDATA.Text)) then begin
        writeln(SMTP.FullResult.Text);
        SMTP.Free;
        exit(false);
     end;
     ...
     if not SMTP.MailData(FILEDATA) then begin
     writeln(SMTP.FullResult.Text);
        SMTP.Free;
        exit(false);
     end;
