...
constructor install_squidclamav.Create();
begin
libs:=tlibs.Create;
...
//#########################################################################################
procedure install_squidclamav.Free();
begin
  libs.Free;
...
//#########################################################################################
procedure install_squidclamav.xinstall();
var
   CODE_NAME:string;
   cmd:string;
   zdate:string;
...
if not FileExists(CC) then begin
 ...
 fpsystem(cmd);
  ...
  if FileExists('/usr/bin/squidclamav') then begin
