...
var
   path:string;
begin
   path:=SYS.LOCATE_GENERIC_BIN('apxs2-prefork');
   if FileExists(path) then exit(path);
   path:=SYS.LOCATE_GENERIC_BIN('apxs2');
   if FileExists(path) then exit(path);
   path:=SYS.LOCATE_GENERIC_BIN('apxs');
   if FileExists(path) then exit(path);
...
//##############################################################################
procedure setupopengoo.MOD_QOS();
var
   CODE_NAME:string;
   cmd:string;
   apxs2:string;
...
begin
  ...
  if not FileExists(apxs2) then begin
     writeln('apxs2 no such file or directory');
  ...
  cmd:=apxs2+' -q LIBEXECDIR >/tmp/artica-LIBEXECDIR.tmp 2>&1';
  fpsystem(cmd);
  l:=Tstringlist.Create;
  l.LoadFromFile('/tmp/artica-LIBEXECDIR.tmp');
  LIBEXECDIR:=l.Strings[0];
  l.free;
  if not DirectoryExists(LIBEXECDIR) then begin
     writeln('LIBEXECDIR:',LIBEXECDIR,' No such directory');
     exit;
  end;

  if FileExists(LIBEXECDIR+'/mod_qos.so') then begin
     writeln('Already installed on ',LIBEXECDIR,'/mod_qos.so');
     exit;
  end;


  CODE_NAME:='APP_MOD_QOS';
  install.INSTALL_STATUS(CODE_NAME,40);
  install.INSTALL_PROGRESS(CODE_NAME,'{compiling}');
  if length(source_folder)=0 then source_folder:=libs.COMPILE_GENERIC_APPS('mod_qos');
  cmd:='cd '+ source_folder +'/apache2 && '+apxs2+' -i -c mod_qos.c';
  writeln(cmd);
  fpsystem(cmd);
   if FileExists(LIBEXECDIR+'/mod_qos.so') then begin
      writeln('Success');
      fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.freeweb.php --httpd');
