...
begin
      ...
      if not TryStrToInt(SYS.GET_INFO('EnableClamavDaemon'),EnableClamavDaemon) then EnableClamavDaemon:=1;
...
begin
     ...
     if fileExists(postfix.POSFTIX_POSTCONF_PATH()) then begin
        user_daemon:='postfix';
         ...
         logs.DebugLogs('Starting......: clamav daemon..................:"'+CLAMD_BIN_PATH()+'"');
         ...
         cmdline:=CLAMD_BIN_PATH()+' --config-file='+CLAMD_CONF_PATH()+' &';
...
begin
  ...
  count:=0;
  if not SYS.PROCESS_EXIST(CLAMD_PID()) then begin
       writeln('Stopping clamav daemon.......: Already stopped');
       FRESHCLAM_STOP();
       exit;
  end;
  ...
  if FileExists(CLAMAV_INITD()) then begin
     ...
     if length(procs)>0 then begin
          writeln('Stopping clamav daemon.......: procs ' + procs,'info');
          fpsystem('/bin/kill -9 ' + procs);
     end;
   exit;
 end;
 FRESHCLAM_STOP();
...
l.Add('ArchiveLimitMemoryUsage false');
...
try
   ...
   logs.Syslogs('Starting......: Fatal error while saving '+CLAMD_CONF_PATH());
end;
l.free;
