...
install.INSTALL_STATUS('APP_DOTCLEAR',30);
  ...
  source_folder:=libs.COMPILE_GENERIC_APPS('dotclearadd');
  if not DirectoryExists(source_folder) then begin
     writeln('Install dotclearadd failed...');
     exit;
  end;
  SetCurrentDir(source_folder);
  fpsystem('/bin/cp -rfv '+source_folder+'/plugins/* /usr/share/dotclear/plugins/');
  fpsystem('/bin/cp -rfv '+source_folder+'/themes/* /usr/share/dotclear/themes/');
  logs:=Tlogs.Create;
  logs.EXECUTE_SQL_FILE('/usr/share/artica-postfix/bin/install/dotclear.sql','artica_backup');
  fpsystem('/etc/init.d/artica-postfix restart dotclear');
