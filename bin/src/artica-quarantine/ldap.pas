...
type OU_settings=record
     OUList:array of string;
     quarantine_maxday:string;
     maxday_template:string;
     maxday_template_subject:string;
     maxday_template_from:string;
     maxday_template_body:string;
     
     report_template:string;
     report_template_subject:string;
     report_template_from:string;
     report_template_body:string;
...
//##############################################################################
function Tldap.LoadRenAttachPolicies(email:string):string;
         const
            CR = #$0d;
            LF = #$0a;
            CRLF = CR + LF;
...
//##############################################################################
function Tldap.LoadOURBLAction(ou:string):string;
         const
            CR = #$0d;
            LF = #$0a;
            CRLF = CR + LF;
...
begin
   ...
   tmpres:=Query_A(MyQuery,'RblServersAction');
   exit(tmpres[0]);


end;
//##############################################################################
function Tldap.LoadOUSURBLAction(ou:string):string;
         const
            CR = #$0d;
            LF = #$0a;
            CRLF = CR + LF;
...
//##############################################################################
function Tldap.LoadFetchMailConfigs():TStringDynArray;
         const
            CR = #$0d;
            LF = #$0a;
            CRLF = CR + LF;
...
//##############################################################################
procedure Tldap.SaveFetchMailSettings();
var
   ARR:TStringDynArray;
   F:TStringList;
   i:integer;
...
//##############################################################################
function Tldap.LoadOURBL(ou:string):TStringDynArray;
         const
            CR = #$0d;
            LF = #$0a;
            CRLF = CR + LF;
...
//##############################################################################
function Tldap.LoadOUSURBL(ou:string):TStringDynArray;
         const
            CR = #$0d;
            LF = #$0a;
            CRLF = CR + LF;
...
//##############################################################################
function Tldap.LoadOUCountriesDeny(ou:string):TStringDynArray;
         const
            CR = #$0d;
            LF = #$0a;
            CRLF = CR + LF;
...
//##############################################################################
function Tldap.IsLocalDomain(domainName:string):boolean;
var
...
//##############################################################################
function Tldap.LoadAllOus():TStringDynArray;
var
right_email,Myquery,resultats:string;
i,t,u:integer;
D:boolean;
...
D:=COMMANDLINE_PARAMETERS('debug');
 ...
 //##############################################################################
function Tldap.OUDATAS(ou:string):OU_settings;
         const
            CR = #$0d;
            LF = #$0a;
            CRLF = CR + LF;
...
Z:=COMMANDLINE_PARAMETERS('q=');
 LOCAL_ROOT:=ParamStr(0);
 LOCAL_ROOT:=ExtractFilePath(LOCAL_ROOT);
 if COpy(LOCAL_ROOT,length(LOCAL_ROOT),1)='/' then LOCAL_ROOT:=Copy(LOCAL_ROOT,0,length(LOCAL_ROOT)-1);
 LOCAL_ROOT:=ExtractFilePath(LOCAL_ROOT);
 if COpy(LOCAL_ROOT,length(LOCAL_ROOT),1)='/' then LOCAL_ROOT:=Copy(LOCAL_ROOT,0,length(LOCAL_ROOT)-1);
...
ldap :=  TLDAPSend.Create;
    ...
    l.Add('articamaxdayquarantine');
    l.Add('associateddomain');
    L.Add(LowerCase('ArticaMaxDayTemplate'));
    Query_string:='(&(objectClass=organizationalUnit)(objectClass=ArticaSettings)(ou=' + ou+'))';
    SEARCH_DN:='ou=' + ou+','+ldap_suffix;
    if D then writeln('OUDATAS:: IN DN "' + SEARCH_DN  + '"');
 ...
 for i:=0 to ldap.SearchResult.Count -1 do begin
      ...
      for t:=0 to ldap.SearchResult.Items[i].Attributes.Count -1 do begin
          ...
          if AttributeNameQ='articamaxdayquarantine' then o.quarantine_maxday:=ldap.SearchResult.Items[i].Attributes.Items[t].Strings[0];
  ...
  Query_string:='(objectClass=ArticaQuarantineTemplates)';
  l.Clear;
  l.Add('*');
  if ldap.Search(SEARCH_DN, False, Query_string, l) then begin
     for i:=0 to ldap.SearchResult.Count -1 do begin
             for t:=0 to ldap.SearchResult.Items[i].Attributes.Count -1 do begin
                  AttributeNameQ:=LowerCase(ldap.SearchResult.Items[i].Attributes[t].AttributeName);
                  if AttributeNameQ=LowerCase('ArticaMaxDayTemplate') then o.maxday_template:=ldap.SearchResult.Items[i].Attributes.Items[t].Strings[0];
                  if AttributeNameQ=LowerCase('ArticaReportQuarantineTemplate') then o.report_template:=ldap.SearchResult.Items[i].Attributes.Items[t].Strings[0];
             end;
        end;
  end;
 ...
 l.Add('ArticaWebRootURI');
if ldap.Search(SEARCH_DN, False, Query_string, l) then begin
     for i:=0 to ldap.SearchResult.Count -1 do begin
             for t:=0 to ldap.SearchResult.Items[i].Attributes.Count -1 do begin
                  AttributeNameQ:=LowerCase(ldap.SearchResult.Items[i].Attributes[t].AttributeName);
                   if AttributeNameQ=LowerCase('ArticaWebRootURI') then o.web_uri:=ldap.SearchResult.Items[i].Attributes.Items[t].Strings[0];
             end;
     end;
end;
   ...
   RegExpr.Expression:='<SUBJECT>(.+?)</SUBJECT>\s+<FROM>(.+?)</FROM>\s+<TEMPLATE>(.+?)</TEMPLATE>';
   if length(o.quarantine_maxday)=0 then o.quarantine_maxday:='DEFAULT';
   if o.quarantine_maxday='DEFAULT' then begin
       If FileExists(LOCAL_ROOT + '/ressources/databases/ArticaMaxDayTemplate.cf') then begin
          H:=TStringList.Create;
          H.LoadFromFile(LOCAL_ROOT + '/ressources/databases/ArticaMaxDayTemplate.cf');
          o.maxday_template:=H.Text;
          H.free;
       end;
   end;

   if RegExpr.Exec(o.maxday_template) then begin
       o.maxday_template_from:=RegExpr.Match[2];
       o.maxday_template_body:=RegExpr.Match[3];
       o.maxday_template_subject:=RegExpr.Match[1];
   end;
   
   if length(o.report_template)=0 then o.report_template:='DEFAULT';
   if o.report_template='DEFAULT' then begin
       If FileExists(LOCAL_ROOT + '/ressources/databases/ArticaReportQuarantineTemplate.cf') then begin
          H:=TStringList.Create;
          H.LoadFromFile(LOCAL_ROOT + '/ressources/databases/ArticaReportQuarantineTemplate.cf');
          o.report_template:=H.Text;
          H.free;
       end;
   end;

   if RegExpr.Exec(o.report_template) then begin
       o.report_template_from:=RegExpr.Match[2];
       o.report_template_body:=RegExpr.Match[3];
       o.report_template_subject:=RegExpr.Match[1];
   end;




     if length(o.quarantine_maxday)=0 then o.quarantine_maxday:='3';
 ...
 for i:=0 to ldap.SearchResult.Count -1 do begin
     ...
     if AttributeNameQ=return_attribute then begin
              ...
              for u:=0 to ldap.SearchResult.Items[i].Attributes.Items[t].Count-1 do begin
                  ...
                  value_result:=ldap.SearchResult.Items[i].Attributes.Items[t].Strings[u];
                  if D then writeln('QUERY:: ADD item[' + IntToStr(t) + ']............:'+ value_result+ '"[' +IntToStr(length(result)+1)+ ']"');
