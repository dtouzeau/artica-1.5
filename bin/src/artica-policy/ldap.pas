...
type
  ...
  public
      ...
      function ISRegisteredDomain(domain:string):boolean;
...
function Tldap.ISRegisteredDomain(domain:string):boolean;
var
resultats,Myquery:string;
begin

  SEARCH_DN:=ldap_suffix;
  Myquery:='(&(ObjectClass=domainRelatedObject)(associatedDomain=' +domain+'))';
  resultats:=trim(Query(MyQuery,'associatedDomain'));
  if length(resultats)>0 then exit(true);

  Myquery:='(&(ObjectClass=transportTable)(cn=' +domain+'))';
  resultats:=trim(Query(MyQuery,'transport'));
  if length(resultats)>0 then exit(true);
  exit(false);
...
ldap :=  TLDAPSend.Create;
    ...
    TRY
      ...
      if D then writeln('QUERY:: EXCEPTION ERROR failed "' + ldap.FullResult + '"');
       ldap.Logout;
       ldap.Free;
       exit;
    end;
