...
begin
   ...
   logs.Debuglogs('DaemonThread1::ParseSharedFolders:: prefix='+ prefix + ' Sources folder=' + INtToStr(sharedfolder_count) + ' [group=' + gpid + ']');


   for i:=0 to sharedfolder_count-1 do begin
       sourcePath:=ini.ReadString('SHARED_FOLDERS','shared.' + IntToStr(i),'');
       logs.Debuglogs('DaemonThread1::ParseSharedFolders::SHARED_FOLDERS/shared.' + IntToStr(i)+'='+sourcepath);
       if FileExists(sourcePath) then begin
              ParseSharedFolders(SourcePath,prefix,ini,gpid);
       end else begin
         logs.Debuglogs('DaemonThread1::ParseSharedFolders: sourcePath "'+sourcePath+'" does not exists');
       end;
