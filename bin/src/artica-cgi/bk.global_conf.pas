...
begin
    ...
    shell('/bin/echo "'+ hostname + '"  >/proc/sys/kernel/hostname');
    l:=TStringList.Create;
    F:=false;
    RegExpr:=TRegExpr.Create;
...
begin
    ...
    result:=ReadFileIntoString('/tmp/hostname.txt');
    result:=trim(result);
...
var
 ...
 begin
   ...
   command_line_curl:=' --progress-bar --output ' + file_path + ' ' + uri;
...
begin
   ...
   if not FileExists(FileToEdit) then  Shell('/bin/touch ' + FileToEdit);
   FileDatas:=TStringList.Create;
...
begin
   ...
   commandline:='/bin/cat -v ' + path + '|grep ''$OpenLDAP:'' >/tmp/ldap_ver 2>&1';
...
begin
  ...
  if SYSTEM_PROCESS_EXIST(pid) then begin
     writeln('Stopping openLdap server pid.: ' + PID + ' PID..');
     if D then writeln('/bin/kill ' + pid);
     shell('/bin/kill ' + pid);
     while SYSTEM_PROCESS_EXIST(pid) do begin
           Inc(count);
           if D then writeln('Stopping openLdap server pid.: ' + PID + ' PID.. (count)',count);
           sleep(100);
           if count>20 then begin
                  writeln('killing OpenLdap server......: ' + PID + ' PID.. (timeout)');
                  shell('/bin/kill -9 ' + PID);
                  break;
           end;
     end;
  end;
...
begin
 ...
 if FileExists('/opt/artica/bin/slapd') then begin
     writeln('Starting......: OpenLDAP server');
     shell('/opt/artica/bin/slapd -f /opt/artica/etc/openldap/slapd.conf');
     pid:=LDAP_PID();
     while not SYSTEM_PROCESS_EXIST(pid) do begin
           ...
           inc(ck);
           if ck>40 then begin
                writeln('failed........: OpenLDAP server');
                exit;
           end;
     end;
     writeln('Started.......: OpenLDAP server ' + pid + ' PID...');
 end;
...
begin
   ...
   if not FileExists(LDAP_GET_SCHEMA_PATH() + '/postfix.schema') then begin
    ...
    shell('/bin/cp ' + ExtractFilePath(ParamStr(0)) + 'install/postfix.schema ' + LDAP_GET_SCHEMA_PATH() + '/postfix.schema');
    Shell( LDAP_GET_INITD() + ' stop');
    Shell( LDAP_GET_INITD() + ' start');
   end else begin
 ...
 if not FileExists(LDAP_GET_SCHEMA_PATH() + '/inetorgperson.schema') then begin
    ...
    shell('/bin/cp ' + ExtractFilePath(ParamStr(0)) + 'install/inetorgperson.schema ' + LDAP_GET_SCHEMA_PATH() + '/inetorgperson.schema');
    Shell( LDAP_GET_INITD() + ' stop');
    Shell( LDAP_GET_INITD() + ' start');
   end else begin
 ...
 if not FileExists(LDAP_GET_SCHEMA_PATH() + '/cosine.schema') then begin
    ...
    shell('/bin/cp ' + ExtractFilePath(ParamStr(0)) + 'install/cosine.schema ' + LDAP_GET_SCHEMA_PATH() + '/cosine.schema');
    Shell( LDAP_GET_INITD() + ' stop');
    Shell( LDAP_GET_INITD() + ' start');
   end else begin
 ...
 if not FileExists(LDAP_GET_SCHEMA_PATH() + '/nis.schema') then begin
    ...
    shell('/bin/cp ' + ExtractFilePath(ParamStr(0)) + 'install/nis.schema ' + LDAP_GET_SCHEMA_PATH() + '/nis.schema');
    Shell( LDAP_GET_INITD() + ' stop');
    Shell( LDAP_GET_INITD() + ' start');
   end else begin
...
if DirectoryExists('/usr/local/var/openldap-data') then begin
   ...
   if FileExists(ExtractFilePath(ParamStr(0)) + 'artica-ldap') then shell(ExtractFilePath(ParamStr(0)) + 'artica-ldap');
...
var admin,password,artica_admin,artica_password,suffix,artica_suffix,queue_source_path:string;
    change:boolean;
...
begin
    ...
    if suffix<>artica_suffix then begin
     ...
     if fileexists('/usr/bin/newaliases') then shell('/usr/bin/newaliases >/dev/null 2>&1');
     if FileExists('/opt/artica/cyrus/bin/reconstruct') then shell('/opt/artica/cyrus/bin/reconstruct >/dev/null 2>&1');
...
var
   ...
   LOGS:Tlogs;
   D:boolean;
   boa_pid,emailrelaycmd,artica_filter_pid,artica_pid,apache_pidn,ldap_pidn,postfix_pidn,crond_pid,artica_policy_pid,mailman_pid:string;
   email_relay_pid,queue_path,fetchmailpid,fetchmailpath:string;
...
begin
     ...
     if D then showscreen('SYSTEM_START_ARTICA_DAEMON:: get boa pid');
     ...
     if D then showscreen('SYSTEM_START_ARTICA_DAEMON:: get apache pid');
     ...
     if not SYSTEM_PROCESS_EXIST(postfix_pidn) then begin
        writeln('Starting......: Postfix daemon...');
     ...
     if FileExists('/opt/artica/mailman/mail/mailman') then begin
        if not SYSTEM_PROCESS_EXIST(mailman_pid) then begin
           writeln('Starting......: mailman daemon...');
           if D then showscreen('SYSTEM_START_ARTICA_DAEMON:: Start mailman service server ');
           MonShell('/opt/artica/mailman/bin/mailmanctl start >/dev/null 2>&1',true);
           end else begin
...
begin
     ...
     if not FileExists(QueuePath) then begin
        ...
        shell('/bin/chown -R artica:root ' + QueuePath + ' >/dev/null 2>&1');
     end;
        
     
    shell('/bin/chmod 755 /var/log/artica-postfix');
    shell('/bin/chown -R artica:root /var/log/artica-postfix  >/dev/null 2>&1');
    shell('/bin/chown -R artica:root /usr/share/artica-postfix/LocalDatabases  >/dev/null 2>&1');
    shell('/bin/chown -R artica:root /var/quarantines  >/dev/null 2>&1');
    shell('/bin/chown -R artica:root ' + QueuePath + ' >/dev/null 2>&1');

    if FileExists('/usr/local/bin/dspam') then begin
         shell('/bin/chown artica:root /usr/local/bin/dspam >/dev/null 2>&1');
         shell('/bin/chown -R artica:root /etc/dspam >/dev/null 2>&1');
         ForceDirectories('/usr/local/var/dspam/data >/dev/null 2>&1');
         ForceDirectories('/var/spool/dspam >/dev/null 2>&1');
         shell('/bin/chown -R artica:root /var/spool/dspam >/dev/null 2>&1');
         shell('/bin/chown -R artica:root /usr/local/var/dspam >/dev/null 2>&1');
    end;
...
begin
  ...
  shell('/bin/ps ax | awk ''{if (match($5, ".*/kavmilter$") || $5 == "$KAV_MILTER") print $1}'' >/tmp/kav6.pids');
  if not FileExists('/tmp/kav6.pids') then exit;
  filedatas:=TStringList.Create;
  filedatas.LoadFromFile('/tmp/kav6.pids');
  RegExpr:=TRegExpr.Create;
  RegExpr.Expression:='([0-9]+)';
  if filedatas.Count=0 then begin
     writeln('stopping......: Kaspersky Mail server already stopped..');
     exit;
     filedatas.free;
     RegExpr.free;
  end;
  For i:=0 to filedatas.Count-1 do begin
     if RegExpr.Exec(filedatas.Strings[i]) then begin
          writeln('stopping......: Kaspersky Mail server (' + RegExpr.Match[1] + ')');
          shell('/bin/kill ' + RegExpr.Match[1]);
     end;
...
pidlists:='';
  ...
  filedatas:=TStringList.Create;
  RegExpr:=TRegExpr.Create;
  if FileExists('/tmp/kav6.pids') then   filedatas.LoadFromFile('/tmp/kav6.pids');
  For i:=0 to filedatas.Count-1 do begin
     if RegExpr.Exec(filedatas.Strings[i]) then pidlists:=pidlists + RegExpr.Match[1] + ' ';
  end;
   ...
   shell('/etc/init.d/kavmilterd start >/tmp/kav6.start 2>&1');
   filedatas.LoadFromFile('/tmp/kav6.start');
   writeln('Starting......: Kaspersky Mail server ' + filedatas.Strings[0]);
   filedatas.free;
...
begin
    ...
    shell('/opt/artica/bin/artica-www -v >/tmp/apache.version 2>&1');
    l:=TstringList.Create;
    l.LoadFromFile('/tmp/apache.version');
    RegExpr:=TRegExpr.Create;
...
begin
  ...
  shell('/opt/artica/bin/php-config  --extension-dir >/tmp/tmp_php5_ext_dir');
  result:=trim(ReadFileIntoString('/tmp/tmp_php5_ext_dir'));
end;
...
if SYSTEM_PROCESS_EXIST(pid) then begin
   ...
   if fileExists('/usr/sbin/postfix') then shell('/usr/sbin/postfix stop >/dev/null 2>&1');
   if fileExists('/etc/init.d/postfix') then shell('/etc/init.d/postfix stop >/dev/null 2>&1');
  end;
...
if FileExists('/etc/init.d/saslauthd') then begin
     shell('/etc/init.d/saslauthd stop >/dev/null 2>&1');
end;

if FileExists('/opt/artica/bin/saslauthd') then begin
     writeln('Starting......: saslauthd');
     shell('/opt/artica/bin/saslauthd -a ldap -n 0 -c -O /opt/artica/etc/saslauthd.conf');
     exit;
end else begin
   if FileExists('/etc/init.d/saslauthd') then begin
     writeln('Starting......: saslauthd');
     shell('/etc/init.d/saslauthd start >/dev/null 2>&1');
   end;
end;
...
begin
   ...
   if SYSTEM_PROCESS_EXIST(CYRUS_PID()) then begin
      ...
      writeln('Starting......: cyrus-imapd');
   if FileExists('/opt/artica/cyrus/bin/master') then begin
      shell('/opt/artica/cyrus/bin/master -d');
      while not SYSTEM_PROCESS_EXIST(CYRUS_PID()) do begin
       ...
       writeln('Started.......: cyrus-imapd PID '+ CYRUS_PID() + '...');
         exit;
...
begin
   ...
   if FileExists(CYRUS_GET_INITD_PATH()) then begin
         shell(CYRUS_GET_INITD_PATH() + ' stop >/dev/null 2>&1');
   end;
         ...
         if RegExpr.Exec(mDatas.Strings[i]) then begin
            ...
            if fileExists(local_path + '/' +  ExtractFileName(RegExpr.Match[1])) then begin
                    ...
                    Shell('/bin/mv ' + FilePathName + ' ' + FilePathNameTO);
                     ...
                     shell('/bin/chown root ' + FilePathNameTO + ' >/dev/null 2>&1');
...
begin
     ...
     line:=ExecPipe('/opt/artica/mailman/bin/version 2>&1');
     ...
     if not fileExists(mainfile) then begin
       ...
       exit;
        ...
        if FileExists(bounce_template_cf) then begin
            ...
            shell('/bin/chown root:root /etc/postfix/bounce.template.cf >/dev/null 2>&1');
        end;
...
begin
        ...
        logs.logsPostfix('POSFTIX_VERIFY_MAINCF:inet_interfaces=' + inet_interfaces);

        if length(inet_interfaces)=0 then begin
           logs.logsPostfix('POSFTIX_VERIFY_MAINCF:inet_interfaces is null change to "inet_interfaces=all"');
        ...
        if FileExists('/etc/postfix/bounce.template.cf') then shell('/bin/chown root:root /etc/postfix/bounce.template.cf >/dev/null 2>&1');
        
        
        mailbox_transport:=POSTFIX_EXTRACT_MAINCF('mailbox_transport');
        logs.logsPostfix('POSFTIX_VERIFY_MAINCF:mailbox_transport=' + mailbox_transport);
        if FileExists(CYRUS_DELIVER_BIN_PATH()) then begin
              if mailbox_transport<>'cyrus' then begin
                     logs.logsPostfix('POSFTIX_VERIFY_MAINCF:Change mailbox transport to cyrus');
                     shell('/usr/sbin/postconf -e "mailbox_transport=cyrus"');
                     shell('/usr/sbin/postconf -e "virtual_transport=cyrus"');
                     MustRestart:=true;
              end;
        end;
        ...
        if DirectoryExists('/var/spool/postfix') then begin
            ...
            shell('/bin/cp /etc/services /var/spool/postfix/etc/services >/dev/null 2>&1');
            shell('/bin/cp /etc/resolv.conf /var/spool/postfix/etc/resolv.conf >/dev/null 2>&1');
            shell('/bin/cp /etc/hosts /var/spool/postfix/etc/hosts >/dev/null 2>&1');
            shell('/bin/cp /etc/localtime /var/spool/postfix/etc/localtime >/dev/null 2>&1');
            shell('/bin/cp /etc/nsswitch.conf /var/spool/postfix/etc/nsswitch.conf >/dev/null 2>&1');
            Shell('/bin/chmod -R 0755 /var/spool/postfix/etc >/dev/null 2>&1');
        end;
...
if SYSTEM_PROCESS_EXIST(pid) then begin
   if fileExists('/usr/sbin/postfix') then shell('/usr/sbin/postfix reload >/dev/null 2>&1');
   if fileExists('/etc/init.d/postfix') then shell('/etc/init.d/postfix reload >/dev/null 2>&1');
   end
   else begin
       if fileExists('/usr/sbin/postfix') then shell('/usr/sbin/postfix start >/dev/null 2>&1');
       if fileExists('/etc/init.d/postfix') then shell('/etc/init.d/postfix start >/dev/null 2>&1');
end;
...
POSFTIX_VERIFY_MAINCF();
if SYSTEM_PROCESS_EXIST(pid) then begin
   if fileExists('/usr/sbin/postfix') then shell('/usr/sbin/postfix stop >/dev/null 2>&1 && /usr/sbin/postfix start >/dev/null 2>&1');
   if fileExists('/etc/init.d/postfix') then shell('/etc/init.d/postfix stop >/dev/null 2>&1 && /etc/init.d/postfix start >/dev/null 2>&1');
   end
   else begin
       if fileExists('/usr/sbin/postfix') then shell('/usr/sbin/postfix start >/dev/null 2>&1');
       if fileExists('/etc/init.d/postfix') then shell('/etc/init.d/postfix start >/dev/null 2>&1');
...
var queue:string;
begin
   Shell('/usr/sbin/postconf -h queue_directory >/tmp/queue_directory');
   queue:=trim(ReadFileIntoString('/tmp/queue_directory'));
...
begin
 ...
 if FileExists(CronTaskkDelete) then begin
  ...
  if FileExists(CronTaskPath + '/artica.cron.kasupdate') then shell('/usr/local/ap-mailfilter3/bin/enable-updates.sh');
   ...
   shell('/bin/chown root:root /etc/cron.d/* >/dev/null 2>&1');
  ...
  if FileExists('/etc/lsb-release') then begin
      if not FileExists('/etc/redhat-release') then begin
             ...
             for i:=0 to  Filedatas.Count-1 do begin
                 ...
                 if RegExpr.Exec(Filedatas.Strings[i]) then distri_provider:=trim(RegExpr.Match[1]);
                 RegExpr.Expression:='DISTRIB_RELEASE=([0-9\.]+)';
                 if RegExpr.Exec(Filedatas.Strings[i]) then distri_ver:=trim(RegExpr.Match[1]);
                 RegExpr.Expression:='DISTRIB_CODENAME=(.+)';
                 if RegExpr.Exec(Filedatas.Strings[i]) then distri_name:=trim(RegExpr.Match[1]);
             end;
...
begin
	...
	main_cf.add('virtual_transport_maps=ldap:/etc/postfix/ldap-virtual_transport_maps.cf');
	...
	main_cf.add('header_checks=regexp:/etc/postfix/hash_files/header_checks.cf');
	...
	main_cf.add('mynetworks=127.0.0.0/24,192.168.5.0/24');
	...
	main_cf.add('VirtualAliasMapsTable_bind_dn =cn=' + ldap_admin +','+ ldap_suffix);
	main_cf.add('VirtualAliasMapsTable_bind_pw =' + ldap_password );
	main_cf.add('VirtualAliasMapsTable_search_base =cn=mailman,cn=artica,' + ldap_suffix );
	main_cf.add('VirtualAliasMapsTable_timeout = 10');
	main_cf.add('VirtualAliasMapsTable_query_filter =(&(objectClass=ArticaMailManRobots)(cn=%s))');
	main_cf.add('VirtualAliasMapsTable_version =3');
	main_cf.add('VirtualAliasMapsTable_result_attribute =cn');
	main_cf.add('#----------------------------------------------------------------------');
	...
	main_cf.add('#----------------------------------------------------------------------');
        ...
        main_cf.SaveToFile('/etc/postfix/main.cf');
        POSTFIX_RESTART_DAEMON();
...
var
   ldap_admin,ldap_password,ldap_server,ldap_suffix,certs:string;
   LOG:Tlogs;
   sys:TSystem;
   ask:string;
   list2:TstringList;
...
///usr/local/etc/saslauthd.conf
       ...
       Shell('/bin/chmod 750 /var/run/cyrus');
       Shell('chmod -R 755 /var/lib/cyrus');
       Shell('chmod -R 755 /var/spool/cyrus');

       shell('/bin/chown -R cyrus:mail /var/lib/cyrus >/dev/null 2>&1');
       shell('/bin/chown -R cyrus:mail /var/spool/cyrus >/dev/null 2>&1');
       shell('/bin/chown -R cyrus:mail /var/run/cyrus >/dev/null 2>&1');
