...
if not FileExists('/tmp/artica-ifconfig.tmp') then begin
   binpath:=LOCATE_GENERIC_BIN('ifconfig');
   fpsystem(binpath+' -a >/tmp/artica-ifconfig.tmp 2>&1');
end;

l:=TstringList.Create;
l.LoadFromFile('/tmp/artica-ifconfig.tmp');
    ...
    for i:=0 to ll.Count-1 do begin
        if RegExpr.Exec(ll.Strings[i]) then begin
           if DirectoryExists('/proc/'+RegExpr.Match[1]) then begin
              ...
              if PROCESS_ISCHROOTED(RegExpr.Match[1]) then continue;
...
begin
  ...
  if Index < ALen then SetLength(Result, Index); // wirkliche LÃ¤nge festlegen
end;
//#########################################################################################
