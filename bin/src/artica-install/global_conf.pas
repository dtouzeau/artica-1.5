     ...
     for i:=0 to l.Count-1 do begin
        ...
        if proc_partition then RegExpr.Expression:='^[A-Za-z]+\s+(.+?):\s+([0-9\.]+)\s+([A-Za-z]+)';
         if RegExpr.Exec(l.Strings[i]) then begin
                  ...
                  if DiskDev='size (logical/physical)' then continue;
...
begin
   ...
   if debug then writeln('Starting instanciate libraries');
    ...
    ldap:=topenldap.Create;
    try
       ArrayList.Add('[APP_OPENLDAP] "'+ldap.LDAP_VERSION()+'"');
    except
          logs.Syslogs('FATAL ERROR on CGI_ALL_APPLIS_INSTALLED after ldap.LDAP_VERSION()');
    end;
   ...
   //openEMM
    ...
    try
       ArrayList.Add('[APP_OPENSSH] "'+sshd.VERSION()+'"');
    except
    ...
    try
       ArrayList.Add('[APP_OPENDKIM] "'+dkfilter.VERSION()+'"');
    except
          logs.Syslogs('FATAL ERROR on CGI_ALL_APPLIS_INSTALLED after dkfilter.VERSION()');
    end;
    dkfilter.free;
    ...
    logs.Debuglogs('postfix.POSTFIX_VERSION()');
...
logs.DebugLogs('Starting......: Boa hostname '+hostname);
...
begin
    ...
    if FileExists('/etc/mysql/debian-start') then fpsystem('/bin/mv /etc/mysql/debian-start /etc/mysql/debian-old-start');
    ...
    try ldap:=topenldap.Create; ldap.LDAP_START(); finally end;
     ...
     writeln('Starting......: eMailRelay instances....');
     emailrelay.START();
     emailrelay.free;
...
begin
 ...
 if NetWorkAvailable then begin
     if FileExists(postfix.POSFTIX_POSTCONF_PATH()) then begin
        ...
        //assp
        logs.Debuglogs('############ opendkim ##################');
        opendkim:=tdkfilter.Create(SYS);
        opendkim.START();
        opendkim.free;
...
l:TstringList;
begin
if not SYS.COMMANDLINE_PARAMETERS('--force') then if fileExists('/etc/init.d/artica-boa') then exit;
l:=TstringList.Create;
l.Add('#!/bin/sh');
l.Add('#Begin /etc/init.d/artica-postfix');
 ...
 if fileExists('/sbin/chkconfig') then begin
    ...
    l.Add('# Short-Description: Start artica-boa daemon');
    l.Add('# chkconfig: 2345 11 89');
    l.Add('# description: artica-boa Daemon');
    l.add('### END INIT INFO');
...
if not SYSTEM_PROCESS_EXIST(BOA_DAEMON_GET_PID()) then begin
   ...
   logs.Debuglogs('Starting......: ' + logs.ReadFromFile(get_ARTICA_PHP_PATH() + '/ressources/logs/boa.start'));
    ...
    logs.DebugLogs('Starting......: Boa done...');
end;
...
begin
  ...
  if ParamStr(2)='--inline' then begin
   ...
   writeln(mysqld_safe+' --skip-grant-tables --skip-external-locking');
   writeln('');
