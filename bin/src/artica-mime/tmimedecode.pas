...
uses
Classes, SysUtils,variants,IniFiles,RegExpr in 'RegExpr.pas',mimemess,mimepart,ldap,smtpsend,unix,Process,logs,strutils,md5;
...
begin
       ...
       logs:=Tlogs.Create;
       commandLines:=FULL_COMMANDLINE_PARAMETERS();
...
//##############################################################################
function ArticamimeDecode.SendHTMLMail():boolean;
var
  RegExpr:TRegExpr;
  T,L,Z:TStringList;
  mm:TMIMEMess;
...
begin
   ...
   if RegExpr.Exec(CommandLines) then mailfrom:=RegExpr.Match[1];
   RegExpr.Expression:='--to=(.+?)\s+';
   if RegExpr.Exec(CommandLines) then mailto:=RegExpr.Match[1];
   RegExpr.Expression:='--subject=(.+?)\s+--';
   if RegExpr.Exec(CommandLines) then subject:=trim(RegExpr.Match[1]);
   RegExpr.Expression:='--content=(.+?)\s+';
   if RegExpr.Exec(CommandLines) then FilePath:=RegExpr.Match[1];



   T:=TStringlist.Create;
   if FileExists(filepath) then T.LoadFromFile(filepath);
   
   writeln('TO: ',mailto);
   writeln('FROM: ',mailfrom);
   writeln('Subject: ',subject);
   ...
   firstpatlogs:='from=<'+mailfrom+'>, to=<'+mailto+'>, original-size='+intTostr(Length(mm.Lines.Text))+' ';

   if FIleExists('/usr/sbin/sendmail') then begin
       filetemp:=logs.FILE_TEMP();
       logs.WriteToFile(mm.Lines.Text,filetemp);

       fpsystem('/usr/sbin/sendmail -bm -t -f '+mailfrom+' <'+filetemp);
       logs.Syslogs(firstpatlogs+' via sendmail');
       exit;
   end;
   
 if not SMTP.MailFrom(RegExpr.Match[1],Length(mm.Lines.Text)) then begin
       logs.Syslogs(firstpatlogs+trim(SMTP.FullResult.Text));
       exit(false);
     end;
