...
uses
Classes, SysUtils,Process,strutils,IniFiles,unix,artica_send_logs,RegExpr in 'RegExpr.pas',md5,ldap,mailinfos,
...
begin
  ...
  for i:=0 to DirListFiles.Count-1 do begin
      ...
      sqLOGS.ArticaSendSql(DirListFiles.Strings[i] + ' --> ' + dsTest.ReturnString);
  ...
  writeln('');
   ...
   for i:=0 to infos.MAILMAN_ROBOTS.Count-1 do begin
       if RegExpr.Exec(infos.MAILMAN_ROBOTS.Strings[i]) then begin
            logs.ArticaSend('mailman:: ' + email + ' is a mailman robot for list "' + RegExpr.Match[1] + '" type=' + RegExpr.Match[3] + ' ou=' + RegExpr.Match[2] );
            writeln('RBL Action............:',infos.RBL_SERVER_ACTION);
  ...
  While Not Eof(input) do begin
    ...
    logs.ArticaSend('X-Spam Tests results: ' +IntToStr(messageinfo.X_SpamTest_Rate));
     ...
     if BOGOFILTER_ROBOTS(messageinfo) then begin
        logs.ArticaSend('BOGOFILTER robot detected...');
        logs.ArticaSend('------------------------------------------------------------------------------------------------> > > END');
        logs.ArticaSend('');
        logs.ArticaSend('');
        exit;
    ...
    if messageinfo.mailman_robot then begin
       logs.ArticaSend('MAILMAN robot detected...');
...
begin
  ...
  if ldap_infos.user_ou=messageinfo.ldap.user_ou then begin
     logs.ArticaSend(messageinfo.real_mailfrom +' is trusted sender (has a local user) ' + ldap_infos.user_ou + '=' + messageinfo.ldap.user_ou);
     exit(true);
  end;
...
begin
   ...
   for i:=0 to messageinfo.ldap.MAILMAN_ROBOTS.Count-1 do begin
       if RegExpr.Exec(messageinfo.ldap.MAILMAN_ROBOTS.Strings[i]) then begin
            ...
            logs.ArticaSend('mailman:: ' + messageinfo.mail_to + ' is a mailman robot for list "' + messageinfo.mailman_listname+ '" type="' + messageinfo.mailman_listtype + '" ou="' + messageinfo.ldap_ou + '"');
            messageinfo.Artica_results:='send';
            messageinfo.mailman_robot:=true;
            result:=messageinfo;
            break;
       end else begin
           //logs.ArticaSend('mailman:: ' +messageinfo.ldap.MAILMAN_ROBOTS.Strings[i] + ' failed for "' + RegExpr.Expression + '"');
       end;
   
   end;
  if messageinfo.mailman_robot=false then logs.ArticaSend('MAILMAN:: <' + messageinfo.mail_to + '> not a mailman robot in ' + IntToStr(messageinfo.ldap.MAILMAN_ROBOTS.Count) + ' robots list ..');
  RegExpr.Free;
...
begin
    ...
    ldap:=Tldap.Create();
             ...
             logs.ArticaSend('ReleaseMail:: ' + email.mail_to + ' actions number=' + IntToStr(MAILTO_LIST_ACTION.count));
             quarantine:=false;
             ...
             if email.mailman_robot=true then logs.ArticaSend('ReleaseMail:: This mail is generated to mailman robot ' +tmp);
      ...
      if length(tmp)>0 then begin
       ...
       //************ BigHtml ****************************************************
       logs.ArticaSend('ReleaseMail:: Big Html rule ???.....');
       BigHtmlMail(email);


     //---------------- message is mailman ------------------------------
       if email.mailman_robot=true then begin
            logs.ArticaSend('ReleaseMail:: This email is a mailman robot, terminate communication...');
            DeleteMailTO(recipient,email);
            email.InputMethodSend:=False;
       end;
...
begin
    ...
    sqLOGS.ArticaSendSql(sql+'=' +DatabasePath + '  "' + dsTest.ReturnString + '"');
    sqLOGS.ArticaSend(dsTest.ReturnString);
    RegExpr.Expression:='SQLITE_DONE';
...
begin
     ...
     Logs.ArticaSend('BuildActionTable:: <' + recipient + '> --->' +email.Artica_results + log_add);
...
begin
 ...
 //************ Extensions cheking ****************************************************
   logs.ArticaSend('ApplyRules:: RENATTACH ???.....');
 ...
 //************ Antivirus ****************************************************
        if not FileExists('/opt/kav/5.6/kavmilter/bin/kavmilter') then begin
           logs.ArticaSend('ApplyRules:: ANTIVIRUS ???.....');
 ...
 //************ WhiteList ****************************************************
         logs.ArticaSend('ApplyRules:: WHITE LIST ???.....');
 ...
 //************ anti-spam ****************************************************
        logs.ArticaSend('ApplyRules:: ANTI-SPAM ???.....');
 ...
 //************ bogofilter ****************************************************
       logs.ArticaSend('ApplyRules:: BOGOFILTER ???.....');
  ...
  //************ Faked sender ****************************************************
       ...
       logs.ArticaSend('ApplyRules:: FAKED ???.....');
  ...
  //************ Country Deny ****************************************************
        logs.ArticaSend('ApplyRules:: COUNTRY DENY ???.....');
 ...
 //************ RBL****************************************************
         logs.ArticaSend('ApplyRules:: RBL ???.....');
 ...
 //************ SURBL****************************************************
         logs.ArticaSend('ApplyRules:: SURBL ???.....');
...
//************ User defined rules ****************************************************
        logs.ArticaSend('ApplyRules:: User defined rules ???.....');
...
//************ mailman ****************************************************
       logs.ArticaSend('ApplyRules:: MAILMAN ???.....');
        if messageinfo.mailman_robot=true then begin
              ScanMailMan(messageinfo);
              BuildActionTable(messageinfo,recipient);
              exit(messageinfo);
         end else begin
             logs.ArticaSend('ApplyRules:: ' + recipient + ' Not a mailman robot...');
         end;
...
begin
    if not FileExists('/opt/artica/mailman/mail/mailman') then begin
       logs.ArticaSend('ScanMailMan:: unable to stat /opt/artica/mailman/mail/mailman (aborting)');
     ...
     if email.mailman_listtype='main' then begin
            ...
            logs.ArticaSend('ScanMailMan:: executing ' + cmd);
            shell(cmd);
            exit(email);
     end;
...
begin
   ...
   db.QuickQuery(sql);
   logs.ArticaSend('EDIT_RBL_CACHE:: ' +db.ReturnString);
   logs.ArticaSendSql(sql + ' => ' + db.ReturnString);
   db.Close;
   db.Free;
end;
...
begin
   if not FileExists(ExtractFilePath(ParamStr(0)) + 'mhonarc') then begin
          logs.ArticaSend('BigHtmlMail:: ' + ExtractFilePath(ParamStr(0)) + 'mhonarc doesn''t exists');
          exit;
   end;
   ...
   logs.ArticaSend('BigHtmlMail:: html block is enabled');
   for i:=0 to Email.ldap.html_block.Rules.Count-1 do begin
     ...
     if RegExpr.Exec(rules) then begin
        logs.ArticaSend('BigHtmlMail:: rule number ' + intTostr(i) + ' From:' + RegExpr.Match[1] + ' To:' + RegExpr.Match[2] + ' Size:' + RegExpr.Match[3] + ' mb');
           if MessageSize>=StrToInt(RegExpr.Match[3]) then begin
              ...
              if RegExpr.Exec(Email.mail_from) then begin
                    ...
                    if RegExpr.Exec(Email.mail_to) then begin
                        logs.ArticaSend('BigHtmlMail:: rule number ' + intTostr(i) + ' match');
                        BigHtmlMail_Backup(email);
                    end;
              end;
           end else begin logs.ArticaSend('BigHtmlMail:: rule number ' + intTostr(i) + ' doesn''t match size "' + IntToStr(MessageSize) + '"');end;
     end;
...
if not FileExists(destination_file) then begin
  ...
  if not FileExists(email.FilePath_source) then begin
         logs.ArticaSend('BigHtmlMail:: ' + email.FilePath_source + ' does not exists');
         exit;
  end;
  ...
  fpsystem(cmd);
  if not FileExists(destination_file) then begin
     logs.ArticaSend('BigHtmlMail:: Unable to stat ' +destination_file);
     exit;
  end;
end;
...
//+ '?path='+email.MessageMD5 + '&org='+email.ldap_ou end else URI:=hostname + '/' + email.ldap_ou + '/'+ email.MessageMD5 + '
 ...
 logs.ArticaSend('BigHtmlMail:: building MAIL_DATAS ' + IntToStr(email.HeaderDatas.Count) + ' header lines, ' + IntToStr(HTML_DATAS.Count) + ' html lines');
 ...
 logs.ArticaSend('BigHtmlMail:: Rebuild the message done....');
...
begin
 ...
 if email.InputMethodSend=false then begin
  ...
  mail_to:=HEAD.Strings[1];
     ...
     LOGS.ArticaSend('ReleaseMail:: init ok');
     if not SMTP.Login then begin
