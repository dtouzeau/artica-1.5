...
function XHRConnection() {
	...
	try {
		...
		if (debug) { alert('Erreur lors de la tentative de creation de l\'objet \nnew XMLHttpRequest()\n\n' + error); }
		try {
	...
	this.createXMLObject = function() {
		try {
			...
			try {
				...
				if (debug) { alert('Erreur lors de la tentative de creation de l\'objet XML\n\n'); }
				return false;
			}
		}
		return xmlDoc;
	...
	this.setXMLObject = function(obj) {
		if (obj == undefined) {
				if (debug) { alert('Parametre manquant lors de l\'appel de la methode setXMLObject'); }
				return false;
		}
		try {
			...
			xmlObj = obj;
				...
				if (debug) { alert('Erreur lors de l\'affectation de l\'objet XML dans la methode setXMLObject'); }
		}
	}
	...
	this.sendAndLoad = function(Url, httpMode, callBack) {
		...
		conn.onreadystatechange = function() {
			if (conn.readyState == 4 && conn.status == 200) {
				...
				// Si une zone destinee a recuperer le resultat a ete definie
		...
		switch(httpMode) {
			case "GET":
				try {
					...
					if (debug) { alert('Echec lors de la transaction avec ' + Url + ' via la methode GET'); }
					return false;
				}
			break;
			case "POST":
   ...
   switch(httpMode) {
      ...
      case "POST":
         try {
            ...
            if (debug) { alert('Echec lors de la transaction avec ' + Url + ' via la methode POST'); }
            return false;
         }
      break;
      default :
