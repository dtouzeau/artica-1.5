...
if(isset($_GET["external-import-resource"])){save_ressource();exit;}
if(isset($_GET["external-export-progress"])){buildstatus();exit;}
if(isset($_GET["external-import-launch"])){LaunchImport();exit;}
...
function js(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body('{wizard_import_config}',"index.export.php");
	$confirm=$tpl->_ENGINE_parse_body('{wizard_import_confirm}',"index.export.php");
	$error_no_ressource=$tpl->_ENGINE_parse_body("{ERROR_NO_RESOURCE_SELECTED}");
...
var x_{$prefix}StartImportProgress= function (obj) {
	var results=obj.responseText;
	if(results.length>0){
	...
	XHR.appendData('external-import-launch','yes');
	XHR.sendAndLoad('$page', 'GET',x_{$prefix}StartExportLaunch);	
	
}

var x_{$prefix}StartImport= function (obj) {
	var results=obj.responseText;
	if(results.length>0){alert(results);}
	var XHR = new XHRConnection();
	XHR.appendData('external-export-progress','yes');
	XHR.sendAndLoad('$page', 'GET',x_{$prefix}StartImportProgress);
}	
...
function StartImport(){
	...
	if(pattern.length>0){
		if(confirm('$confirm')){
		var XHR = new XHRConnection();
		XHR.appendData('external-import-resource',pattern);
		XHR.sendAndLoad('$page', 'GET',x_{$prefix}StartImport);
		}
	}else{
		alert('$error_no_ressource');
	}
}
...
function start_page(){
	...
	$sock=new sockets();
	$resource=$sock->GET_INFO('InstantImportExternalResource');
	$usb="Loadjs('browse.usb.php?set-field=external-export-resource');";
	
	$html="<H1>{wizard_import_config}</H1>
	
	<table style='width:100%'>
	<tr>
		<td valign='top'><img src='img/128-import.png'>
		<td valign='top'>
			<div id='export-progress'>
			<p class=caption>{wizard_import_config_step1}</p>
			<p class=caption>{wizard_import_config_step2}</p>
			</div>
			<table style='width:100%'>
			<tr>
				<td class=legend>{selected_resource}:</td>
				<td><input type='text' id='external-export-resource' name='external-export-resource' value='$resource'\" style='width:240px'></td>
				<td><strong>/artica-export.tar.gz</strong>
			</tr>
			...
			<table style='width:100%'>
				...
				<tr>
					<td colspan=2 align='right'>
						<hr>
						<input type='button' OnClick=\"javascript:StartImport();\" value='{launch_importation}&nbsp;&raquo;'>
					</td>
				</tr>
				</table>
	...
	$tpl=new templates();
	echo $tpl->_ENGINE_parse_body($html,"index.export.php");
...
function save_ressource(){
	$sock=new sockets();
	$sock->SET_INFO('InstantImportExternalResource',$_GET["external-import-resource"]);
...
function LaunchImport(){
	$sock=new sockets();
	$sock->getfile('LaunchImportConfiguration');
...
function logs(){
	$tpl=new templates();
	$html=$tpl->_ENGINE_parse_body("<H1>{wizard_import_config} {events}</H1>","index.export.php");
