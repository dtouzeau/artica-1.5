...
if($nofork){
	...
	shell_exec(trim($nohup." {$GLOBALS["NICE"]}".$unix->LOCATE_PHP5_BIN()." ".dirname(__FILE__)."/exec.parse-orders.php >/dev/null 2>&1 &"));
	die();
...
function watchdog_me(){
	...
	if($GLOBALS["TOTAL_MEMORY_MB"]<400){
		...
		$cmd=trim($nohup." {$GLOBALS["NICE"]}".$unix->LOCATE_PHP5_BIN()." ".dirname(__FILE__)."/exec.parse-orders.php >/dev/null 2>&1 &");
		shell_exec($cmd);
		$cmd=trim($nohup." {$GLOBALS["NICE"]}".$unix->LOCATE_PHP5_BIN()." ".__FILE__." --all >/dev/null 2>&1 &");
		shell_exec($cmd);
		$cmd=trim($nohup." {$GLOBALS["NICE"]}/etc/init.d/artica-postfix restart fcron >/dev/null 2>&1 &");
		shell_exec($cmd);
		return;
	}
	$sock=new sockets();
	...
	if($time_file>3){
		...
		shell_exec("{$GLOBALS["nohup"]} {$GLOBALS["NICE"]}/etc/init.d/artica-postfix restart artica-status >/dev/null 2>&1 &");
...
function squid_master_status(){
	...
	if(!$GLOBALS["CLASS_UNIX"]->process_exists($pid)){
		...
		if($GLOBALS["CLASS_UNIX"]->process_exists($pid)){@file_put_contents($pidpath, $pid);}
	}
...
function clamd(){
	...
	if($timeTimeFile>5){
		...
		$time=time();
		if(!is_dir("/var/log/artica-postfix/clamd-mem")){@mkdir("/var/log/artica-postfix/clamd-mem",755,true);}
...
function winbindd(){
	...
	shell_exec("{$GLOBALS["PHP5"]} {$GLOBALS["NICE"]}/usr/share/artica-postfix/exec.samba.php --ping-ads &");
