...
***************************************************************************/
//
// $Id: class.HP-UX.inc.php,v 1.27 2008/05/27 13:58:32 bigmichi1 Exp $
//
class sysinfo {
  ...
  public function chostname() {
    if (execute_program('hostname', '', $ret)) {
      return $ret;
    } else {
      return 'N.A.';
    }
  ...
  public function kernel() {
    if (execute_program('uname', '-srvm', $ret)) {
      return $ret;
    } else {
      return 'N.A.';
    }
  ...
  public function ide() {
    ...
    foreach($bufd as $file) {
      if (preg_match('/^hd/', $file)) {
        ...
        if (rfts("/proc/ide/" . $file . "/capacity", $buf, 1)) {
  ...
  public function usb() {
    ...
    $devstring = 0;
    $devnum = -1;
    if (rfts('/proc/bus/usb/devices', $bufr)) {
      $bufe = explode("\n", $bufr);
      foreach($bufe as $buf) {
        if (preg_match('/^T/', $buf)) {
          $devnum+= 1;
